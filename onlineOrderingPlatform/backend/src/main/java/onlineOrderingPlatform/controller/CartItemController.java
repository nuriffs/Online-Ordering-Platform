...
//	// Remove all items in the cart
    ...
    public ResponseEntity<?> getCartItem(@PathVariable Long cartItemId) {
        ...
        if (cartItem != null) {
            return ResponseEntity.status(HttpStatus.OK).body("Cart item found: " + cartItem.toString());
        } else {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Cart item with ID " + cartItemId + " not found");
        }
    }

    @PutMapping("/update/{cartItemId}")
    public ResponseEntity<?> updateCartItem(@PathVariable Long cartItemId, @RequestBody CartItem updatedCartItem) {
        CartItem cartItem = cartItemService.updateCartItem(cartItemId, updatedCartItem);
        
        if (cartItem != null) {
        	return ResponseEntity.status(HttpStatus.OK).body("Cart item updated: " + cartItem.toString());
        }else {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Cart item with ID " + cartItemId + " not found");
        }
    }
